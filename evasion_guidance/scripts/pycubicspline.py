...
def calc_2d_spline_interpolation(x, y, v, num=100):
    """
    ...
    for i_s in s:
        ...
        r_u.append(sp.calc_u(i_s, v))

    travel = np.cumsum([np.hypot(dx, dy) for dx, dy in zip(np.diff(r_x), np.diff(r_y))]).tolist()
    travel = np.concatenate([[0.0], travel])

    return r_x, r_y, r_yaw, r_k, travel, r_u

def calc_2d_spline_interpolation_fixed_distance(x, y, v, dist=5):
    """
    Calc 2d spline course with interpolation
